...
use llm_bucket::synchronise::{
    SynchroniseReport, ExternalSourceReport, ExternalItemReport
};
...
async fn test_synchronise_readme_to_pdf_upload() {
    ...
    assert!(!report.sources.is_empty(), "At least one source should be reported");
    for src in &report.sources {
        assert!(!src.items.is_empty(), "Each source should have at least one item");
        assert!(src.source_id > 0, "Source id should be positive");
        assert!(!src.source_name.is_empty(), "Source name should not be empty");
...
async fn test_empty_bucket_removes_all_sources() {
    ...
    let client = LLMClient::new_from_env()
    ...
    let _ = client.create_source(NewExternalSource {
    ...
    llm_bucket::synchronise::empty_bucket(&client).await.expect("empty_bucket should succeed");

    // Assert: bucket should be empty
    let sources = client.list_sources().await.expect("list_sources should succeed");
    assert!(sources.is_empty(), "Bucket should have no sources after empty_bucket");
}
...
async fn test_synchronise_removes_existing_sources_before_upload() {
    ...
    let dummy_source = client.create_source(NewExternalSource {
        name: &dummy_name,
        bucket_id: bucket_id as i32,
    }).await.expect("Failed to create dummy source");

    // Ensure dummy source was created
    let sources = client.list_sources().await.expect("list_sources should succeed");
    assert!(sources.iter().any(|src| src.external_source_id == dummy_source.external_source_id), "Dummy source should exist before sync");
    ...
    let sources_after = client.list_sources().await.expect("list_sources should succeed after");
    assert!(
        sources_after.iter().all(|src| src.external_source_id != dummy_source.external_source_id),
        "Dummy source should be removed after synchronise"
    );
    // At least one source must exist, and none are the dummy
    assert!(!sources_after.is_empty(), "At least one source (new) should remain after");
}
...
async fn test_synchronise_flattenfiles_uploads_codebase_files() {
    ...
    assert!(!report.sources.is_empty(), "At least one source should be reported for FlattenFiles");
    for src in &report.sources {
        assert!(!src.items.is_empty(), "Each source should have at least one item (FlattenFiles)");
        assert!(src.source_id > 0, "Source id should be positive in FlattenFiles");
        assert!(!src.source_name.is_empty(), "Source name should not be empty in FlattenFiles");
        for item in &src.items {
            assert!(item.item_id > 0, "Item id should be positive in FlattenFiles");
            assert!(!item.item_name.is_empty(), "Item name should not be empty in FlattenFiles");
        }
    }
}
